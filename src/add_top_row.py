...
def add_rows():
    ...
    for table in rahshal.tables:
      ...
      make_table_format_as_required(table)
      first_row(table)
      style_the_docx_table(table)
      table_count += 1
      print(f"Table {table_count} of {len(rahshal.tables)} modified")
...
def make_table_format_as_required(table):
